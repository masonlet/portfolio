...
function validateForm(form: HTMLFormElement): ContactBody {
  ...
  if (!email || !subject || !message) throw new Error("All fields are required.");
...
function initForm(): void {
  ...
  if (!(statusDiv instanceof HTMLElement)) throw new Error(
  ...
  form.addEventListener('submit', async (e: SubmitEvent) => {
    ...
    catch (error) { statusDiv.textContent = error instanceof Error
                                              ? error.message 
